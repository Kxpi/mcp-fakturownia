...
- **lookup_company_by_nip** — Wyszukiwanie w rejestrze po NIP (biała lista VAT + CEIDG); zwraca `suggested_create_payload` do `create_client`
...
2. `lookup_company_by_nip` — sprawdź `warnings` (status VAT, nieaktywny CEIDG, adres)
